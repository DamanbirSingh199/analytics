...
#Clustering works on a distance algorithm
#Works on numerical data, but can't know the distance on some parameters
#Grouping is done by within the cluster distance should be least and similiar, among clusters maximium
...
k1 <- amap::Kmeans(data[,-1],centers=3, iter.max = 200)#k means cluster, want 3 cluster, iterate maximum 200 times
...
k1$withinss
###Fetch the cluster for each obs, lesser distance more cohesive
#k1$cluster
k1$cluster
k1$centers
...
table(k1$cluster)
k1$size
data_clus_2 <- data[ k1$cluster == 2,]#These are the customr IDs of a particular cluster
(data_clus_2)
